...
class CarRentalController extends Controller
{
    ...
    public function storeRental(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'bike_id' => 'required|integer|exists:bike_rentals,id', // ID xe
            'customer_name' => 'required|string|max:255', // Tên khách hàng
            'customer_email' => 'required|email|max:255', // Email khách hàng
            'customer_phone' => 'required|string|max:20', // Số điện thoại khách hàng
            'pick_up_date' => 'required|date', // Ngày nhận xe
            'return_date' => 'required|date|after_or_equal:pick_up_date', // Ngày trả xe
            'pick_up_location_id' => 'required|integer|exists:locations,id', // Địa điểm nhận xe
            'return_location_id' => 'required|integer|exists:locations,id', // Địa điểm trả xe
            'driver_license_number' => 'required|string|max:50', // Số bằng lái xe
            'note' => 'nullable|string', // Ghi chú
        ]);
        ...
        $bike = BikeRental::find($data['bike_id']);
        ...
        $rental = Rental::create([
            ...
            'pick_up_location_id' => $data['pick_up_location_id'],
            'return_location_id' => $data['return_location_id'],
            'driver_license_number' => $data['driver_license_number'],
            'note' => $data['note'] ?? null,
            'status' => 'ongoing', // Trạng thái thuê xe
        ]);
    ...
    public function search(Request $request)
    {
        // Validate dữ liệu đầu vào
        $validator = Validator::make($request->all(), [
            'pick_up_location' => 'required|integer|exists:locations,id',
            'pick_up_date' => 'required|date',
            'return_location' => 'nullable|integer|exists:locations,id',
