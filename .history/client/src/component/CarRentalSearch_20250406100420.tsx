...
const BikeCard = ({ bike }: { bike: any }) => (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
        ...
        />
        <div className="p-4">
            <h3 className="text-lg font-bold text-gray-800 truncate">{bike.bike_name}</h3>
...
const BikeList = () => {
    ...
    useEffect(() => {
        const getBikes = async () => {
            try {
                ...
                setBikes(data?.data); // Đảm bảo cấu trúc của data đúng
            } catch (err) {
    ...
    if (loading) return <p className="text-center text-gray-600">Đang tải danh sách xe máy...</p>;
    if (error) return <p className="text-center text-red-500">{error}</p>;

    return (
        <div className="my-10">
            <h1 className="text-3xl font-bold text-center text-gray-800 mb-8">Danh sách xe máy</h1>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 px-4">
                {bikes.map((bike) => (
                    <Link to={`/bike-detail/${bike.id}`} key={bike.id} className="block">
                        <BikeCard bike={bike} />
                    </Link>
                ))}
            </div>
